...
{
    public partial class Userinfo : CustomerPageAction
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (DataConvert.CLng(mu.ParentUserID) > 0) { pmu = buser.SelReturnModel(Convert.ToInt32(mu.ParentUserID)); }
                Dinfo.Add(mu);
                Iinfo.Add(basemu);
        ...
        protected void UInfo_RPT_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.CommandName == "useLogin")
            {
                ...
                Response.Write("<script>window.open('/user/Default')</script>");
            }
        }
        //已发布文章数
