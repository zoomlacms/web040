        ...
        protected void Auditing()
        {
            ...
            if (chkArr != null)
            {
                for (int i = 0; i < chkArr.Length; i++)
                {
                    ...
                    //是否是认证信息节点
                    if(mcom.NodeID==536|| mcom.NodeID == 537)
                    {
                        string tablename = "";
                        if(mcom.NodeID==536)
                        {
                        ...
                        DataTable dt = SqlHelper.ExecuteTable(CommandType.Text, "select * from " + tablename + " where ID=" + mcom.ItemID, null);
                        if (dt.Rows.Count > 0)
                        {
                            userinfo.GroupID = DataConvert.CLng(dt.Rows[0]["groupid"]);
                            buser.UpdateByID(userinfo);
                        }
                    }
                }
            }//For end;
