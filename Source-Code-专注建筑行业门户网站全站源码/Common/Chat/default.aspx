<%@ Page Language="C#" AutoEventWireup="true" CodeFile="default.aspx.cs" Inherits="ZoomLaCMS.Common.Chat.chat1"  ClientIDMode="Static" ValidateRequest="false" EnableViewState="false" MasterPageFile="~/Common/Master/Empty.master" %>
...
<li role="presentation" class="active"><a href="#home" aria-controls="home" role="tab" data-toggle="tab">公司人员</a></li>
...
<ItemTemplate>
<li onclick="ChangeTalker(<%#"'"+Eval("UserID")+"','"+Eval("UserName")+"','"+Eval("UserFace")+"'" %>);" class="media" id="list_item_<%#Eval("HoneyName") %>">
<div class="media-left"><img src="<%#Eval("UserFace") %>" class="media-object" onerror="shownoface(this);" /></div>
...
欢迎[<asp:Label runat="server" ID="UserName_L" />]，您可以通过UID来指定聊天对象，如：/Common/Chat/default.aspx?uid=1[<a href="javascript:;" onclick="copyToClipBoard();">复制会议室邀请</a>] [<a href="javascript:;" onclick="ShowModal();">退出系统</a>]
...
function GetToReg() {
    if (parent) { parent.location = "/User/Register?ReturnUrl=<%=Request.RawUrl %>"; } else { location = "/User/Register.aspx?ReturnUrl=<%=Request.RawUrl %>"; }
}
function AJAXLogin() {
