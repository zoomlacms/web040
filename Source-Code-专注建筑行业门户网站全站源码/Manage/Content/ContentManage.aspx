<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ContentManage.aspx.cs" Inherits="ZoomLaCMS.Manage.I.ASCX.ContentManage" MasterPageFile="~/Manage/I/Default.master" ValidateRequest="false"%>
<asp:Content runat="server" ContentPlaceHolderID="head">
<title><%=Resources.L.内容管理 %></title>
    <link rel="stylesheet" href="/dist/css/font-awesome.min.css" />
    <style>.codepop{max-width:200px;width:181px;}</style>
</asp:Content>
...
<ZL:Repeater ID="RPT" runat="server" PageSize="10" PagePre="<tr><td><label class='allchk_l'><input type='checkbox' id='chkAll'/>全选</label></td><td colspan='8'><div class='text-center'>"
	...
	<ItemTemplate>
		<tr ondblclick="location='ShowContent.aspx?GID=<%#Eval("GeneralID") %>';" data-order="<%#Eval("OrderID") %>" data-gid="<%#Eval("GeneralID") %>">
			...
			<td>
                <a class="precode" href="javascript:;" <%#Eval("Status").ToString().Equals("99") ? "":"hidden" %> data-gid='<%#Eval("GeneralID") %>' onmouseover="ShowCode(this);" onmouseout="HideCode();" ><i class="fa fa-qrcode"></i></a>
...
<div>
	...
	<asp:Button ID="btnAudit" CausesValidation="false" runat="server" Text="<%$Resources:L,审核通过 %>" CssClass="btn btn-info" OnClick="btnAudit_Click" OnClientClick="if(!IsSelectedId()){alert('请选择审核项');return false;}else{return confirm('你确定要审核选中内容吗？')}" />
	<asp:Button ID="btnUnAudit" CausesValidation="false" runat="server" Text="<%$Resources:L,取消审核 %>" CssClass="btn btn-info" OnClick="btnUnAudit_Click" OnClientClick="if(!IsSelectedId()){alert('请选择审核项');return false;}else{return confirm('你确定要取消审核选中内容吗？')}" />
...
<script type="text/javascript">
	...
	function ShowCode(ele) {
	    var $ele = $(ele);
	    if ($ele.find(".codepop").length <= 0) {
	        var code = '/common/common.ashx?url=<%=ZoomLa.Components.SiteConfig.SiteInfo.SiteUrl%>/Item/' + $ele.data("gid") + '.aspx';
	        var tlp = '<div class="codepop popover fade left in" style="" ><div class="arrow" style="top: 50%;"></div><h3 class="popover-title">扫描二维码浏览</h3><div class="popover-content"><div><img src="' + code + '" style="width:150px;height:150px;"></div></div></div>';
	        $ele.prepend($(tlp));
	    }
	    $ele.find(".codepop").show();
	}
    //function HideCode() { $(".codepop").hide();}
